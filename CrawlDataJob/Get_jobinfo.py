...
def get_profile_info_24(driver, url):
    try:
        ...
        requirements = get_Requirement_24(page_source)
        rights = get_Right_24(page_source)
        deadline = get_Deadline_24(page_source)
        salary = get_Salary_24(page_source)
        address = get_Place_24(page_source)
        level = get_Level_24(page_source)
        numberEmployee = get_Way_24(page_source)
        sourcePicture = get_SrcPic_24(page_source)
        ...
        return [title, company_name, salary, dateSub, deadline, address, numberEmployee, level, description, requirements, rights, headquater, sourcePicture, company_place, company_scale]
    except Exception as e:
